...
message_1001 = list2vec([one,0,0,one])
#encoding_1001 = G*message_1001
...
t3_codeword = list2vec([0,one,one,one,one,0,0])
R*t3_codeword
...
def find_error(syndrome):
    ...
    Examples:
        ...
        True
    """
    assert(syndrome.D == H.D[0])
    if syndrome == zero_vec(syndrome.D):
        return Vec(H.D[1], {})

    val = -1
    for i in range(3):
        val += pow(2, i)*(1 if (syndrome[2-i] == one) else 0)
    return Vec(H.D[1], {val:one})

## Task 6
# Use the Vec class for your answers.
non_codeword = Vec({0,1,2,3,4,5,6}, {0: one, 1:0, 2:one, 3:one, 4:0, 5:one, 6:one})
error_vector = Vec({0,1,2,3,4,5,6}, {6: one}) # find_error(H*non_codeword)
code_word = Vec({0,1,2,3,4,5,6}, {0:one, 2:one, 3:one, 5:one}) # error_vector + non_codeword
...
def find_error_matrix(S):
    ...
    Example:
        ...
        True
    """
    return coldict2mat({k:find_error(col) for k,col in mat2coldict(S).items()})
...
## Ungraded Task
CTILDE = C + noise(C, 0.01)
bits2str(mat2bits(H*CTILDE))
